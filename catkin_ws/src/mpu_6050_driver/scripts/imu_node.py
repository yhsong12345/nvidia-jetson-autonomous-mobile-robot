...
def publish_imu(timer_event):
    ...
    imu_msg.linear_acceleration.x = accel_x / 682.072
    imu_msg.linear_acceleration.y = accel_y / 682.072
    imu_msg.linear_acceleration.z = accel_z / 682.072
    
    # ok
    imu_msg.angular_velocity.x = (gyro_x *0.0174) if abs(gyro_x / 1024.0) > 0.01  else 0
    imu_msg.angular_velocity.y = (gyro_y *0.0174) if abs(gyro_y / 1024.0) > 0.01  else 0
    imu_msg.angular_velocity.z = (gyro_z *0.0174) if abs(gyro_z / 1024.0) > 0.01  else 0
